...
from tensorflow.keras.layers import Layer, Conv1D, Multiply, Add, TimeDistributed, ZeroPadding2D, Concatenate, Dropout, LeakyReLU
...
class Interactor(Layer):
    def __init__(self, n_splits, hidden_size, kernel_size, groups, regular, dropout, **kwargs):
...
class SCINet(Layer):
    def __init__(self, n_splits, n_levels, hidden_size, kernel_size, groups, regular, dropout, **kwargs):
    ...
    def rev_split(self, sci_output):
        while len(sci_output) > 1:
            ...
            for i in range(steps):
                tmp_sci_reversed.append( Concatenate(1)(sci_output[i::steps]) )
